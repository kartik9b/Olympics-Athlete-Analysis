...
# -----------------------------------------------------------------------------
# 2. CACHED DATA LOADING (PARQUET)
# -----------------------------------------------------------------------------
DATA_PATH = Path(__file__).parent / "athlete_data.parquet"
...
def load_data() -> pd.DataFrame:
    if not DATA_PATH.exists():
        st.error(
            f"❌ Could not find `{DATA_PATH.name}` in the repository root directory."
        )
        st.stop()
...
# Filter 1: Season
seasons = ["All"] + list(df["Season"].dropna().unique())
selected_season = st.sidebar.selectbox("Select Season", seasons)

# Filter 2: Year Range Slider
min_year, max_year = int(df["Year"].min()), int(df["Year"].max())
selected_years = st.sidebar.slider(
    "Select Year Range", min_year, max_year, (1980, max_year)
)

# Filter 3: Sport Dropdown
sports = ["All"] + sorted(list(df["Sport"].dropna().unique()))
...
# -----------------------------------------------------------------------------
st.markdown(
    "<h1 class='main-title'>Olympic History & Performance Analytics</h1>",
    unsafe_allow_html=True,
)
st.caption(
    f"Analyzing **{selected_years[0]} - {selected_years[1]}** | Filtered Entries: **{len(filtered_df):,}**"
)
...
with col1:
    st.metric(
        label="Unique Athletes", value=f"{filtered_df['ID'].nunique():,}"
    )
...
with col4:
    female_pct = (
        filtered_df["Sex"].value_counts(normalize=True).get("F", 0)
    ) * 100
...
tab1, tab2, tab3 = st.tabs(
    ["📈 Participation Trends", "🏆 Medal Standings", "📊 Physical Attributes"]
)
...
with tab1:
    ...
    gender_trend = (
        filtered_df.groupby(["Year", "Sex"])["ID"].nunique().reset_index()
    )
...
with tab2:
    ...
    if not medal_df.empty:
        ...
        top_nocs_df = (
            medal_df[medal_df["NOC"].isin(top_10_nocs)]
